...
await page.evaluate(() =>
  localStorage.setItem(
    "ooo_progress_v2",
    JSON.stringify({ unlocked: [], selectionCounts: {}, gamesCompleted: 3 }),
  ),
);
  ...
  page.waitForFunction(
    (k) => {
      const s = window.__game.scene
        .getScenes(true)
        .find((s) => s.scene.key === k);
      return (
        !!s &&
        !s.tweens
          .getTweens()
          .some((t) => t.data && t.data.some((d) => d.key === "x"))
      );
    },
    k,
...
const btn = (k, r) =>
  page.evaluate(
    ([k, r]) => {
      ...
      const walk = (l, ox, oy) => {
        for (const o of l) {
          if (o.list && o.list.some((c) => c.text && rx.test(c.text)))
            return { x: ox + o.x, y: oy + o.y };
          if (o.list) {
            const h = walk(o.list, ox + o.x, oy + o.y);
            if (h) return h;
          }
        }
      };
...
const stats = (fr) => {
  const s = [...fr].sort((a, b) => b - a);
  return `worst ${s.slice(0, 5).join(", ")} | over-16ms: ${fr.filter((f) => f > 16).length}/${fr.length}`;
};
...
await settle("Items");
console.log(
  "cards built right after entrance:",
  await page.evaluate(() => {
    const s = window.__game.scene
      .getScenes(true)
      .find((s) => s.scene.key === "Items");
    return s.gallery
      ? `${s.gallery.built.size}/${s.gallery.items.length}`
      : "n/a";
  }),
);
...
await settle("Items");
console.log(
  "objects drawn twice check — main-camera-visible cards:",
  await page.evaluate(() => {
    const s = window.__game.scene
      .getScenes(true)
      .find((s) => s.scene.key === "Items");
    const main = s.cameras.main;
    return s.gallery.track.list.filter((c) => !(c.cameraFilter & main.id))
      .length;
  }),
);
await browser.close();
